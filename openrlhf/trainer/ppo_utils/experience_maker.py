...
class RemoteExperienceMaker(BaseExperienceMaker):
    ...
    def compute_advantages_and_returns(
        ...
        # get complexity
        complexity = [experience.info["response_length"] for experience in experiences]
        ...
        if args.advantage_estimator == "rloo":
            ...
            rewards = torch.cat(rewards).reshape(-1, args.n_samples_per_prompt).to(device="cuda")
            if args.use_bonus:
                complexity = torch.cat(complexity).reshape(-1, args.n_samples_per_prompt).to(device="cuda")
                com_mask = rewards.trunc()
                correct_com = complexity * com_mask
                correct_com[correct_com == 0] = float('inf')
                min_com, min_indices = torch.min(correct_com, dim=-1)
                mask = min_com > args.bonus_constraint
                rewards[mask, min_indices[mask]] += args.bonus_reward_coef
            
            if args.use_dyna_weight:
                difficulty = rewards.trunc().sum(-1, keepdim=True) / args.n_samples_per_prompt
                difficulty = difficulty.detach()
                dc = 4 * args.diff_coef * difficulty * (1 - difficulty)
